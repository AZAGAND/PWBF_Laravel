                        ...
                        @forelse ($pets as $i => $pet)
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    ...
                                    <p class="text-sm text-gray-500">{{ $pet->warna_tanda }}</p>
                                </td>
                                <td class="py-5 px-4">
                                ...
                                <td class="py-5 px-4">
                                    <span class="inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium {{ $pet->jenis_kelamin == 'Jantan' ? 'bg-blue-600 text-blue-600' : 'bg-pink-600 text-white' }}">
                                        {{ $pet->jenis_kelamin }}
                                    </span>
                                </td>
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('hewan.edit', $pet->idpet) }}" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-blue-100 hover:text-blue-600 dark:bg-gray-700 dark:text-white dark:hover:bg-blue-900/50 dark:hover:text-blue-400 transition" title="Edit">
                                        ...
                                        {{-- Delete --}}
                                        <form action="{{ route('hewan.destroy', $pet->idpet) }}" method="POST" onsubmit="return confirm('Hapus data hewan {{ $pet->nama }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Belum ada data hewan.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
