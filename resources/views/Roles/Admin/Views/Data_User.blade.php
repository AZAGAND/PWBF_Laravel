                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        @if (Route::has('users.edit'))
                                            <a href="{{ route('users.edit', $u->iduser) }}" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-blue-100 hover:text-blue-600 dark:bg-gray-700 dark:text-white dark:hover:bg-blue-900/50 dark:hover:text-blue-400 transition" title="Edit">
                                        ...
                                        @if (Route::has('users.password'))
                                            <a href="{{ route('users.password', $u->iduser) }}" onclick="return confirm('Ganti password?')" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-yellow-100 hover:text-yellow-600 dark:bg-gray-700 dark:text-white dark:hover:bg-yellow-900/50 dark:hover:text-yellow-400 transition" title="Ganti Password">
                                                 <svg class="fill-current" width="16" height="16" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <rect x="2" y="9" width="14" height="7" rx="2" stroke="currentColor" stroke-width="2"/>
                                                    <path d="M5 9V5C5 2.79086 6.79086 1 9 1C11.2091 1 13 2.79086 13 5V9" stroke="currentColor" stroke-width="2"/>
                                                </svg>
                                                Password
                                            </a>
                                        @endif
                                        ...
                                        @if (Route::has('users.destroy'))
                                            <form action="{{ route('users.destroy', $u->iduser) }}" method="POST" onsubmit="return confirm('Hapus user {{ $u->nama }}?');" class="inline-block">
                                                @csrf
                                                @method('DELETE')
