    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-title-md2 font-bold text-black dark:text-white">
                Dashboard Data Kode Tindakan Terapi
            </h2>
        ...
        <div class="rounded-sm border border-gray-200 bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-gray-800 dark:bg-gray-900 sm:px-7.5 xl:pb-1">
            ...
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-xl font-semibold text-gray-800 dark:text-white">
                        List Data Kode Tindakan Terapi
                    </h3>
                </div>
                
                <div>
                    @if (Route::has('kode_tindakan_terapi.trash'))
                        <a href="{{ route('kode_tindakan_terapi.trash') }}"
                    ...
                    <a href="{{ route('kode_tindakan_terapi.create') }}"
            ...
            <div class="max-w-full overflow-x-auto">
                <table class="w-full table-auto">
                    ...
                    <tbody>
                        @forelse ($kode_tindakan_terapis as $i => $item)
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('kode_tindakan_terapi.edit', $item->idkode_tindakan_terapi) }}" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-blue-100 hover:text-blue-600 dark:bg-gray-700 dark:text-white dark:hover:bg-blue-900/50 dark:hover:text-blue-400 transition" title="Edit">
                                        ...
                                        {{-- Delete --}}
                                        <form action="{{ route('kode_tindakan_terapi.destroy', $item->idkode_tindakan_terapi) }}" method="POST" onsubmit="return confirm('Hapus data {{ $item->kode }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Belum ada data kode tindakan terapi.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
