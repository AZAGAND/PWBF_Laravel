...
@section('content')
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="rounded-sm border border-gray-200 bg-white shadow-default dark:border-gray-800 dark:bg-gray-900">
            ...
            <form action="{{ route('dokter.update', $dokter->idrole_user) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="p-6.5">
                    ...
                    <div class="mb-4.5">
                        ...
                        <div class="relative z-20 bg-transparent dark:bg-form-input">
                            <select name="iduser" required
                                ...
                                {{-- Include the current user even if they are a doctor (obviously) --}}
                                <option value="{{ $dokter->iduser }}" selected class="text-body dark:text-body dark:bg-gray-800">
                                    {{ $dokter->user->nama }} ({{ $dokter->user->email }}) [Saat Ini]
                                </option>
                                @foreach($users as $user)
                                    <option value="{{ $user->iduser }}" 
                                        {{ old('iduser', $dokter->iduser) == $user->iduser ? 'selected' : '' }}
                    ...
                    <div class="mb-4.5">
                        ...
                        <div class="relative z-20 bg-transparent dark:bg-form-input">
                            <select name="status" required
                                class="relative z-20 w-full appearance-none rounded border border-[1.5px] border-gray-300 bg-transparent py-3 px-5 outline-none transition focus:border-blue-600 active:border-blue-600 dark:border-gray-700 dark:bg-form-input text-black dark:text-white dark:focus:border-blue-500 @error('status') border-red-500 @enderror">
                                <option value="1" {{ old('status', $dokter->status) == 1 ? 'selected' : '' }} class="text-body dark:text-body dark:bg-gray-800">Aktif</option>
                                <option value="0" {{ old('status', $dokter->status) == 0 ? 'selected' : '' }} class="text-body dark:text-body dark:bg-gray-800">Nonaktif</option>
                            </select>
                            <span class="absolute top-1/2 right-4 z-30 -translate-y-1/2">
                                <svg class="fill-current text-gray-500 dark:text-gray-400" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M5.29289 8.29289C5.68342 7.90237 6.31658 7.90237 6.70711 8.29289L12 13.5858L17.2929 8.29289C17.6834 7.90237 18.3166 7.90237 18.7071 8.29289C19.0976 8.68342 19.0976 9.31658 18.7071 9.70711L12.7071 15.7071C12.3166 16.0976 11.6834 16.0976 11.2929 15.7071L5.29289 9.70711C4.90237 9.31658 4.90237 8.68342 5.29289 8.29289Z" fill="currentColor"></path>
