                ...
                <div class="p-6.5">
                    ...
                    <div class="mb-4.5">
                        <label class="mb-2.5 block text-black dark:text-white">
                            User (Akun) <span class="text-red-500">*</span>
                        </label>
                        <div class="relative z-20 bg-transparent dark:bg-form-input">
                            <select name="iduser" required
                                ...
                                <option value="" disabled class="text-body dark:text-body dark:bg-gray-800">Pilih user</option>
                                @foreach($users as $user)
                                    <option value="{{ $user->iduser }}" 
                                        {{ old('iduser', $pemilik->iduser) == $user->iduser ? 'selected' : '' }}
                                        class="text-body dark:text-body dark:bg-gray-800">
                    ...
                    <div class="mb-4.5">
                        ...
                        <input type="text" name="no_wa" required placeholder="Contoh: 081234567890" value="{{ old('no_wa', $pemilik->no_wa) }}"
                            class="w-full rounded border border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-black dark:text-white @error('no_wa') border-red-500 @enderror" />
                        @error('no_wa')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4.5">
                        ...
                        <textarea name="alamat" rows="3" required placeholder="Masukkan alamat lengkap"
                            class="w-full rounded border border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-black dark:text-white @error('alamat') border-red-500 @enderror">{{ old('alamat', $pemilik->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
