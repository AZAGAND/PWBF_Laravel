        ...
        <div class="rounded-sm border border-gray-200 bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-gray-800 dark:bg-gray-900 sm:px-7.5 xl:pb-1">
            ...
            <div class="max-w-full overflow-x-auto">
                <table class="w-full table-auto">
                    ...
                    <tbody>
                        @forelse ($datapemilik as $i => $pemilik)
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('pemilik.edit', $pemilik->idpemilik) }}" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-blue-100 hover:text-blue-600 dark:bg-gray-700 dark:text-white dark:hover:bg-blue-900/50 dark:hover:text-blue-400 transition" title="Edit">
                                        ...
                                        {{-- Delete --}}
                                        <form action="{{ route('pemilik.destroy', $pemilik->idpemilik) }}" method="POST" onsubmit="return confirm('Hapus data pemilik {{ $pemilik->user->nama ?? '' }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Belum ada data pemilik.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
