        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav>
                <ol class="flex items-center gap-2">
                    ...
                    <li>
                        <a class="font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" href="{{ route('data_role') }}">
                            Manajemen Role /
                        </a>
                    </li>
                    ...
                    <tbody>
                        @forelse($roleUsers as $roleUser)
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-5 dark:hover:bg-gray-800">
                                    ...
                                    <div class="flex items-center gap-4">
                                        <div class="h-12 w-12 rounded-md shrink-0">
                                             <div class="h-10 w-10 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-500 dark:text-gray-300 font-bold">
                                                {{ substr($roleUser->user->nama ?? '?', 0, 1) }}
                                            </div>
                                        </div>
                                        <span class="text-sm text-black dark:text-white font-medium">
                                            {{ $roleUser->user->nama ?? 'User tidak ditemukan' }}
                                        </span>
                                    </div>
                                </td>
                                <td class="py-5 px-4">
                                ...
                                <td class="py-5 px-4">
                                    <span class="text-sm text-black dark:text-white font-medium">
                                        {{ $roleUser->role->nama_role ?? 'Role tidak ditemukan' }}
                                    </span>
                                </td>
                                <td class="py-5 px-4">
                                    @if ($roleUser->status)
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex items-center justify-center gap-2">
                                        {{-- Restore --}}
                                        <form action="{{ route('role.restore', $roleUser->idrole_user) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        {{-- Force Delete --}}
                                        <form action="{{ route('role.force_delete', $roleUser->idrole_user) }}" method="POST" onsubmit="return confirm('Hapus permanen role ini?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Tidak ada data role user yang dihapus.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
