        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav>
                <ol class="flex items-center gap-2">
                    ...
                    <li>
                        <a class="font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" href="{{ route('data_kode_tindakan_terapi') }}">
            ...
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-xl font-semibold text-gray-800 dark:text-white">
                        List Kode Tindakan Terapi Terhapus
                    </h3>
                </div>
                            ...
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-5 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Restore --}}
                                        <form action="{{ route('kode_tindakan_terapi.restore', $item->idkode_tindakan_terapi) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        {{-- Force Delete --}}
                                        <form action="{{ route('kode_tindakan_terapi.force_delete', $item->idkode_tindakan_terapi) }}" method="POST" onsubmit="return confirm('Hapus permanen data {{ $item->kode }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Tidak ada data kode tindakan terapi dihapus.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
