    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-title-md2 font-bold text-black dark:text-white">
                Dashboard Data Kategori Klinis
            </h2>
            ...
            <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="text-xl font-semibold text-gray-800 dark:text-white">
                        List Data Kategori Klinis
                    </h3>
                </div>
                
                <div>
                    @if (Route::has('kategori_klinis.trash'))
                        <a href="{{ route('kategori_klinis.trash') }}"
                            ...
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Edit --}}
                                        <a href="{{ route('kategori_klinis.edit', $item->idkategori_klinis) }}" class="inline-flex items-center gap-1.5 rounded bg-gray-100 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-blue-100 hover:text-blue-600 dark:bg-gray-700 dark:text-white dark:hover:bg-blue-900/50 dark:hover:text-blue-400 transition" title="Edit">
                                        ...
                                        {{-- Delete --}}
                                        <form action="{{ route('kategori_klinis.destroy', $item->idkategori_klinis) }}" method="POST" onsubmit="return confirm('Hapus kategori klinis {{ $item->nama_kategori_klinis }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Belum ada data kategori klinis.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
