        ...
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav>
                <ol class="flex items-center gap-2">
                    ...
                    <li>
                        <a class="font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" href="{{ route('data_kategori') }}">
                            Data Kategori /
                        </a>
                    </li>
                            ...
                            <tr class="transition-colors border-b border-[#eee] dark:border-strokedark hover:bg-gray-5 dark:hover:bg-gray-800">
                                ...
                                <td class="py-5 px-4">
                                    <div class="flex flex-wrap items-center justify-center gap-2">
                                        {{-- Restore --}}
                                        <form action="{{ route('kategori.restore', $kategori->idkategori) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('PUT')
                                        ...
                                        {{-- Force Delete --}}
                                        <form action="{{ route('kategori.force_delete', $kategori->idkategori) }}" method="POST" onsubmit="return confirm('Hapus permanen kategori {{ $kategori->nama_kategori }}?');" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-5 px-4 text-center dark:border-strokedark">
                                    <p class="text-gray-500 dark:text-gray-400">Tidak ada data kategori dihapus.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
